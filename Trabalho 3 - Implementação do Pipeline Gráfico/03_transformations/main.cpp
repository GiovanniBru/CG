...
void Display(void) {
    ...
    // Vetores de Base do espaço da câmera: 
    glm::vec3 camDirecao = glm::vec3(0.0f, 0.0f, 0.0f); // Ponto para o qual a câmera aponta = (0,0,0)
    glm::vec3 camPosicao = glm::vec3(-0.1f, 0.1f, 0.25f);// Posição da Câmera = (-1/10, 1/10, 1/4)
    glm::vec3 camUP = glm::vec3(0.0f, 1.0f, 0.0f); // Vetor UP = (0,1,0)
    	// Direção da câmera:
    glm::vec3 camZ = -glm::normalize(camDirecao - camPosicao); 
    glm::vec3 camX = glm::normalize(glm::cross(camUP, camZ));
    glm::vec3 camY = glm::normalize(glm::cross(camZ,camX));
    	// Matriz B da base invertida:
    glm::mat4 B = glm::mat4(glm::vec4(camX.x, camY.x, camZ.x, 0),
 			     glm::vec4(camX.y, camY.y, camZ.y, 0),	    				
 			     glm::vec4(camX.z, camY.z, camZ.z, 0),
 			     glm::vec4(0,0,0,1) 		 );	
 	// Matriz T que translada a base da câmera e faz sua origem coincidir com a do espaço do universo: 		     
    glm::mat4 T = glm::mat4(glm::vec4(1, 0, 0, 0),
 			     glm::vec4(0, 1, 0, 0),	    				
 			     glm::vec4(0, 0, 1, 0),
 			     glm::vec4(-camPosicao.x, -camPosicao.y, -camPosicao.z, 1) 		 );	
...
int main(int argc, char** argv) {
    ...
    // Título da janela
    glutCreateWindow("Resultados Obtidos");
